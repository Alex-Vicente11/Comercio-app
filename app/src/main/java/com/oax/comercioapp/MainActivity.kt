...
class MainActivity : AppCompatActivity() {
  ...
  override fun onCreate(savedInstanceState: Bundle?) {
    ...
    initializeGuestUserIfNeeded()
  }

    private fun initializeGuestUserIfNeeded() {
        if (!SessionManager.isLoggedIn()) {
            authViewModel.guestCreateRequest.observe(this) { result ->
                when (result) {
                    is NetworkResult.Success -> {
                        println("Guest creado: ${result.data.userId}")
                    }
                    is NetworkResult.Error -> {
                        println("Error guest: ${result.message}")
                    }
                    else -> {}
                }
            }
            authViewModel.createGuestUser()
        }
    }
  ...
  private fun setupNavigation() {
    ...
    val appBarConfiguration = AppBarConfiguration(
      setOf(
        R.id.navigation_cart, R.id.navigation_profiles, R.id.navigation_notifications
      )
    )
